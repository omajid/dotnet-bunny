...
namespace Turkey.Tests
{
    public class NuGetTest
    {
        ...
        public async Task LivePackagesAreIdentifiedCorrectly(string version, bool live)
        {
            using (var http = new HttpClient())
            {
                NuGet nuget = new NuGet(http);
                var ver = Version.Parse(version);
                Assert.Equal(live, await nuget.IsPackageLiveAsync(ver, json));
            }
        }

        [Theory]
        [InlineData(new string[] { }, "")]
        [InlineData(new string[] { "https://foo" }, "<add key=\"0\" value=\"https://foo/\" />")]
        [InlineData(new string[] { "https://foo", "file://bar" }, "<add key=\"0\" value=\"https://foo/\" /> <add key=\"1\" value=\"file://bar/\" />")]
        public void NuGetConfigIsGeneratedCorrectly(string[] urls, string feedParts)
        {
            using (var http = new HttpClient())
            {
                ...
                var actualConfig = nuget.GenerateNuGetConfig(urls.Select(x => new Uri(x)).ToList());
